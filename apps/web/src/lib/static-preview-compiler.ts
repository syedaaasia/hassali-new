...
export function compileStaticPreview(input: StaticPreviewCompilationInput): StaticPreviewCompilationResult {
  ...
  const bridge = `<script data-hassali-static-preview-bridge>(()=>{"use strict";const projectId=${escapeInlineScript(JSON.stringify(input.projectId))};const page=${escapeInlineScript(JSON.stringify(activeHtmlPath))};const selectorFor=(node,index)=>{if(node.id)return"#"+node.id;const className=typeof node.className==="string"?node.className.trim().split(/\\s+/).filter(Boolean).slice(0,2).join("."):"";return node.tagName.toLowerCase()+(className?"."+className:"")+":qa("+index+")";};const collect=()=>{const nodes=Array.from(document.querySelectorAll("header,nav,h1,h2,h3,img,button,a,input,select,textarea,[data-menu-toggle],.hero,.hero-stage,.hero-copy"));const elements=nodes.slice(0,160).map((node,index)=>{const rect=node.getBoundingClientRect();const style=getComputedStyle(node);const tag=node.tagName.toLowerCase();const visible=style.display!=="none"&&style.visibility!=="hidden"&&Number(style.opacity)!==0&&rect.width>0&&rect.height>0;const role=tag==="img"?"image":/^h[1-3]$/.test(tag)?"heading":tag==="nav"||tag==="header"?"navigation":"other";const image=tag==="img"?node:null;return{clientWidth:node.clientWidth,complete:image?image.complete:undefined,height:rect.height,interactive:/^(a|button|input|select|textarea)$/.test(tag),naturalHeight:image?image.naturalHeight:undefined,naturalWidth:image?image.naturalWidth:undefined,role,scrollWidth:node.scrollWidth,selector:selectorFor(node,index),visible,width:rect.width,x:rect.left,y:rect.top};});window.parent.postMessage({type:"HASSALI_STATIC_PREVIEW_VISUAL_QA",projectId,observation:{documentHeight:document.documentElement.scrollHeight,documentWidth:document.documentElement.scrollWidth,elements,page,viewportHeight:window.innerHeight,viewportWidth:window.innerWidth}},"*");};let queued=false;const schedule=()=>{if(queued)return;queued=true;requestAnimationFrame(()=>requestAnimationFrame(()=>{queued=false;collect();}));};document.addEventListener("click",event=>{const target=event.target;const anchor=target&&target.closest?target.closest("a[href]"):null;if(!anchor)return;if(anchor.dataset.previewExternal==="true"||anchor.dataset.previewLinkBlocked==="true"){event.preventDefault();return;}const nextPage=anchor.dataset.previewPage;if(!nextPage)return;event.preventDefault();window.parent.postMessage({type:"HASSALI_STATIC_PREVIEW_NAVIGATE",projectId,path:nextPage},"*");});window.addEventListener("load",schedule,{once:true});window.addEventListener("resize",schedule,{passive:true});if(document.readyState==="complete")schedule();})();</script>`;
