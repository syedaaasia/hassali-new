...
export function readDiscoveryState(value: unknown): GrowthDiscoveryState {
  ...
  const validBusiness = v.business === null || (() => { const b = record(v.business); return [b.name, b.offer, b.description, b.valueProposition].every(str) && nullable(b.geography) && nullable(b.website) && evidence(b.evidence) && ["inferred", "user_provided", "website_handoff", "source_only"].includes(String(b.status)); })();
  if (v.analysis !== undefined && (!["not_requested", "provider_blocked", "incomplete", "complete"].includes(String(record(v.analysis).status)) || (record(v.analysis).failureCode !== undefined && !/^GROWTH_[A-Z_]+$/.test(String(record(v.analysis).failureCode))))) return emptyGrowthDiscovery();
