...
const forbiddenPattern = /(?:\b(?:no|without|disable|avoid|exclude|remove)\s+(?:any\s+)?(?:webgl|three(?:\.js)?|threejs|react\s+three\s+fiber|r3f|3d|three-dimensional|canvas)\b|\b(?:webgl|three(?:\.js)?|threejs|3d|canvas)\s+(?:off|disabled|forbidden|removed)\b|\bstatic\s+only\b|\bminimal\s+animation\s+only\b)/i;
