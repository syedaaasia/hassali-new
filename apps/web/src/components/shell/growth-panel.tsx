...
export function GrowthPanel() {
  ...
  const submit = async (action: Action, value = prompt, extras: { audienceId?: string; selectedIds?: string[] } = {}) => {
    ...
    try {
      ...
      setView(action === "capture" ? "Business" : action === "analyze" ? (payload.state.discovery.audiences.length ? "Audiences" : "Business") : action === "outreach" ? "Outreach" : "Companies");
      }
    ...
    <div className={styles.layout}><main className={styles.main}>
      ...
      <div className={styles.content}>
        ...
        {state.job && <details><summary>Discovery diagnostics</summary><dl>
          ...
          <dt>Company-type / geography matches</dt><dd>{state.job.funnel.companyTypeMatches ?? "Not recorded"} / {state.job.funnel.geographyMatches ?? "Not recorded"}</dd>
          <dt>Size matches / unverified</dt><dd>{state.job.funnel.sizeMatches ?? "Not recorded"} / {state.job.funnel.sizeUnverified ?? "Not recorded"}</dd>
        ...
        {!projectId ? <div className={styles.empty}><h2>Select a project</h2><p>Growth discovery belongs to your selected project.</p></div> : hydrating ? <p role="status">Loading Growth...</p> : <>
          {view === "Business" && <><div className={styles.sectionHeading}><h2>{state.business?.name ?? "Grow your business"}</h2>{state.business && <span className={styles.badge}>{state.business.status.replace(/_/g, " ")}</span>}</div>
            {state.business?.status === "source_only" && <p className={styles.notice}>Website title and source excerpts only. Business interpretation and audience generation are not complete.</p>}
            {state.analysis?.failureCode && <p className={styles.notice} role="status">Analysis incomplete ({state.analysis.failureCode}). Saved source evidence remains available.</p>}
            {state.business && <div className={styles.business}><p>{state.business.description}</p><dl><dt>Offer</dt><dd>{state.business.offer || "Not established"}</dd><dt>Value proposition</dt><dd>{state.business.valueProposition || "Not established"}</dd><dt>Market</dt><dd>{state.business.geography ?? "Not established"}</dd></dl>{state.business.evidence.map((e, i) => <blockquote key={i}><p>{e.quote}</p><a href={e.url} target="_blank" rel="noreferrer">{e.field.replace(/_/g, " ")} source</a><p className={styles.muted}>Retrieved {new Date(e.checkedAt).toLocaleString()}</p></blockquote>)}<button className={styles.primary} onClick={() => setView("Audiences")}>Compare audiences</button></div>}
            <form className={styles.businessForm} onSubmit={(e) => { e.preventDefault(); void submit("capture", businessInput); }}><label htmlFor="growth-business">Business website or description</label><textarea id="growth-business" value={businessInput} onChange={(e) => setBusinessInput(inputValue(e))} placeholder="https://your-business.com or what you sell and who you serve" maxLength={4000} rows={3}/><div className={styles.actions}><button className={styles.primary} disabled={!businessInput.trim() || loading} type="submit">Read website</button><button disabled={!businessInput.trim() || loading} type="button" onClick={() => void submit("analyze", businessInput)}>Analyze business with model</button></div></form>
          </>}
          ...
          {view === "Companies" && <><div className={styles.sectionHeading}><h2>Companies <span className={styles.muted}>{companies.length}</span></h2><span className={styles.badge}>{state.discovery.status.replace(/_/g, " ")}</span></div>
            ...
            {chosen && <article className={styles.evidence} aria-label={`Evidence for ${chosen.name}`}><div className={styles.sectionHeading}><h3>{chosen.name}</h3><button onClick={() => setDetail(null)}>Close evidence</button></div><p className={styles.muted}>Checked {new Date(chosen.lastVerifiedAt).toLocaleString()}</p>{chosen.companySize && <p>Company size: {chosen.companySize.value?.label ?? "Not established"}. {chosen.companySize.match === "unverified" ? "Size fit unverified" : chosen.companySize.match === "match" ? "Within requested size" : "Outside preferred size"} ({chosen.companySize.constraintStatus === "confirmed" ? "required constraint" : "inferred preference"}).</p>}<h4>Why this company</h4><ul>{chosen.fitReasons.map((r, i) => <li key={i}>{r}</li>)}</ul><p>Decision-maker roles to verify: {chosen.targetRoles.join(", ") || "Not established"}</p>{chosen.evidence.map((e, i) => <blockquote key={i}><p>{e.quote}</p><a href={e.url} target="_blank" rel="noreferrer">{e.field} source</a></blockquote>)}</article>}
          </>}
...
function SearchCriteria({ plan }: { plan: GrowthSearchPlan }) {
  ...
  if (plan.audienceContext) groups.unshift(["Selected audience", [plan.audienceContext.name]]);
  if (plan.companySize?.value) groups.push([plan.companySize.status === "confirmed" ? "Required company size" : "Company size preference (inferred)", [plan.companySize.value.label]]);
