...
export function GrowthPanel() {
  ...
  return <section className="flex min-h-0 flex-1 flex-col bg-[hsl(var(--premium-panel)/0.35)]" data-growth-runtime>
    ...
    <div className="min-h-0 flex-1 overflow-y-auto px-5 py-6"><div className="mx-auto max-w-3xl">{state?.artifact?.content ? <article className="whitespace-pre-wrap text-[13px] leading-6 text-foreground/90">{state.artifact.content}</article> : <div className="py-16 text-center"><h2 className="text-lg font-semibold">Turn project truth into a focused growth plan</h2><p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-muted-foreground">Growth uses the current applied WEBSITE handoff to prepare positioning, acquisition channels, campaigns, content plans, conversion work, and experiments. It never sends or publishes automatically.</p></div>}{error ? <p className="mt-4 text-sm text-rose-300" role="alert">{error}</p> : null}</div></div>
